...
//检查今日签到弹框
exports.checkqiandaolxjDialog = function () {
    ...
    const notDialogStr = "当前不是今日签到领金币弹框"
    //签到最外面的弹框
    const dialogUi = className(dialogUiClassName).text("今日签到").find()
    ...
    const getMoneyButtom = className(dialogUiClassName).text("立即签到").find()
    const tomorrowButtom = className(dialogUiClassName).descContains("额外").find()
